...
#    membership = models.CharField(max_length=9, choices=MEMBERSHIP_CHOICES)
    ...
    @property
    def admins(self):
        return self.membership.filter(role=3).values_list("user", flat=True)

    @property
    def moderators(self):
        return self.membership.filter(role=2).values_list("user", flat=True)

    @property
    def good_members(self):
        return self.membership.exclude(role=0)
